...
This guide sets up automatic daily runs at 12:00 PM PST, even when your MacBook is locked.
...
2. **Sleep Mode**: If your MacBook goes to sleep, the job will run when it wakes up (if it's past 12:00 PM). For guaranteed execution, keep it plugged in and prevent sleep:
...
4. **Time Zone**: The job is set to run at 12:00 PM **PST/PDT** (America/Los_Angeles). It automatically adjusts for daylight saving time.
...
# Add this line (runs at 12:00 PM PST daily)
0 12 * * * ~/trend-guard/scripts/trendguard_daily.sh >> ~/trend-guard/logs/cron.log 2>&1
```
